...
export const formatChillDecimalNumber = (num: number | string, maximumFractionDigits = 2, minimumFractionDigits = 0) => {
  return formatDecimalNumber(num, maximumFractionDigits, minimumFractionDigits).replaceAll(",", "");
};

export const formatDecimalNumber = (num: number | string, maximumFractionDigits = 4, minimumFractionDigits = 0) => {
...
export const parseMetadata = (metadata: string) => {
  try {
    const { image = "", desc = "", twitter = "", telegram = "", website = "" } = JSON.parse(metadata);
    return {
      image,
      desc,
      twitter,
